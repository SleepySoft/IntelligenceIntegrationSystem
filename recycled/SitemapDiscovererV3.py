...
"""
Sitemap Analyzer (PyQt5) - v3.1 (Fetcher Strategy & ThreadFix)
...
VERSION 3.1 UPDATES:
- Fixed `greenlet.error: Cannot switch to a different thread`.
- Fetcher/Discoverer instances are now created *inside* the QRunnable workers,
  ensuring Playwright is created, used, and destroyed on the same thread.
- GUI no longer holds persistent fetcher/discoverer instances.
- Added `progress` signal to workers for better status updates.
...
class PlaywrightFetcher(Fetcher):
    ...
    Can bypass most anti-bot measures, including JS challenges.
    """
    ...
    def __init__(self, log_callback=print):
        ...
        try:
            self._log("Starting PlaywrightFetcher (Advanced, Slow)...")
            self.playwright = sync_playwright().start()
            # We can add proxy options here if needed
    ...
    def get_content(self, url: str) -> Optional[bytes]:
        try:
            # We create a new context per request to isolate cookies/storage
            ...
            # --- NEW (Stealth Fix) ---
            # Add a script to run *before* the page loads
            # This hides the "navigator.webdriver" flag that anti-bot systems look for
...
# (FIXED: Now creates/destroys Fetcher inside the worker)
#
# =============================================================================
...
class WorkerSignals(QObject):
    ...
    progress = pyqtSignal(str)  # NEW: For sending log messages
...
class ChannelDiscoveryWorker(QRunnable):
    ...
    def run(self):
        ...
        try:
            ...
            if "Advanced (Playwright)" in self.strategy_name:
                if not sync_playwright:
                    raise ImportError("Playwright not installed.")
                fetcher = PlaywrightFetcher(log_callback=log_callback)
            else:
...
class SitemapAnalyzerApp(QMainWindow):
    ...
    def __init__(self):
        ...
        # --- Initialize UI ---
        self.init_ui()
        self.setWindowTitle("Sitemap Channel Analyzer (v3.1 - ThreadFix)")
