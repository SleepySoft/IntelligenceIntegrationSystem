...
> IIS将于2月15日正式切换到v2版本，当前v2版本代码在[dev分支](https://github.com/SleepySoft/IntelligenceIntegrationSystem/tree/dev)。
> 
> v2版本对情报分析结果及评分方式进行了进一步的优化，新的数据结构通用性更好，且对大模型训练更友好。v2版本可以兼容v1版本的数据，反之则不行。
> 
> 该分支将和dev分支分叉，且最终dev分支的v2版本将正式成为main分支，而当前的main分支则以v1的名字成为一个归档用的分支。
> 
> 因此现在大家想自行部署的话，建议直接使用dev分支。该分支上已经修复大量issue，最近对该分支的改进主要追求稳定而非大改。
