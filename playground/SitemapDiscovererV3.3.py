...
"""
Sitemap Analyzer (PyQt5) - v3.3 (playwright-stealth)

A PyQt5 application to browse and analyze a website's sitemap "channels"
and their corresponding article URLs.

VERSION 3.3 UPDATES:
- Replaced problematic 'playwright-extra' with 'playwright-stealth'.
- Added a third fetching strategy: "Stealth (Playwright)".
- PlaywrightFetcher now conditionally applies stealth_sync() for full anti-bot bypass.
- Added granular import error checking.
...
if not sync_playwright or not stealth_sync:
    ...
    if 'stealth_sync' not in locals(): stealth_sync = None
...
class PlaywrightFetcher(Fetcher):
    ...
    def __init__(self, log_callback=print, stealth: bool = False):
        ...
        if not sync_playwright or (stealth and not stealth_sync):
...
class SitemapAnalyzerApp(QMainWindow):
    ...
    def __init__(self):
        ...
        # --- Initialize UI ---
        self.init_ui()
        self.setWindowTitle("Sitemap Channel Analyzer (v3.3 - Stealth)")
